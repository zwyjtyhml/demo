...
class GetRank():
    ...
    def get_rank(self,research_dir_list,article_ra,article_time,patent_ra,referenced_count_rate,downloaded_count_rate):
        ...
        with driver.session() as session:
            ...
            results = []
            for research_dir in research_dir_list:
                datas = session.run(
                    ...
                    if record["per.DBentered"] == 0:
                        ...
                        for resultsPer in resultsPers:
                            if "article" in resultsPer[2]._labels:  # 是文章节点
                                ...
                                atg = CalcuScore().CalActicleScore(resultsPer[2]._properties['sourse'],
                                                                   resultsPer[2]._properties['date'], article_time, 99,
                                                                   referenced_count_rate, 99, downloaded_count_rate)
                                atg_sum = atg_sum + atg
                                at_count = at_count + 1
                            # 专利节点
                            else:
                        ...
                        newlist = [str(result[0]._id), str(result[0]._properties['name']), str(at_count), str(pa_count),
                                   99,
        ...
        for i, j in msgview.iterrows():
            dic = {'author_id': j.author_id, 'author_name': j.author_name, 'author_college': j.author_college,
                   'author_major': j.author_major, 'score': j.score}
            resList.append(dic)
        # print(resList)
